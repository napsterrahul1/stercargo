...
@section('sub_title'){{translate('Edit PRS')}}@endsection
            ...
            <div class="mx-auto col-lg-12">
                <div class="card">
                    ...
                    <form class="form-horizontal" action="{{ route('admin.prs.update-shipment',['shipment'=>$shipment->id]) }}" id="kt_form_1" method="POST" enctype="multipart/form-data">
                    @csrf
                    {{ method_field('PATCH') }}
                        <div class="card-body">
                            <div class="row">
                ...
                @if($user_type == 'admin' || in_array('1005', $staff_permission) )
                    .on('select2:open', () => {
                        $(".select2-results:not(:has(a))").append(`<li style='list-style: none; padding: 10px;'><a style="width: 100%" href="{{route('admin.clients.create')}}?redirect=admin.shipments.create"
                            class="btn btn-primary" >+ {{translate('Add New Customer')}}</a>
                            </li>`);
                    });
                @endif
